            ...
            const float *kernel, int hkw, int hkh, uchar *dst)
{
    ...
    for (j=hkh; j<h-hkh; j++)
    {
        for (i=hkw; i<w-hkw; i++)
        {
            val_r = val_g  = val_b = 0;
            for (n=-hkh; n<=hkh; n++)
            {                                   // src = {00_r,00_g,00_b,10_r,10_g,10_b,...,w0_r,w0_g,w0_b,01_r,01_g,01_b,...wh_r,wh_g,wh_b}
                for (m=-hkw; m<=hkw; m++)
                {                                                   // j = 2, i = 2 n = -1,0,1 m = -1,0,1
                    r = src[(j-n)*w*3+(i-m)*3];                     // src = {...,02_r,02_g,02_b,...,22_r,22_g,22_b,...}
                    g = src[(j-n)*w*3+(i-m)*3+1];                   // 22_r = 2*w*3 + 2*3  22_g = 22_r + 1  22_b = 22_r + 2
                    b = src[(j-n)*w*3+(i-m)*3+2];                   // 12_r = 2*w*3 + (2-1)*3  32_r = 2*w*3 + (2+1)*3
                    val_r +=  r * kernel[(n+hkh)*kw+m+hkw];         // 21_r = (2-1)*w*3 + 2*3  23_r = (2+1)*w*3 + 2*3
