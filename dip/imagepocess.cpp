...
void splitImageChannel(QImage &image, float *r, float *g, float *b)
{
    ...
    for (int j=0; j<height; j++) {
        for (int i=0; i<width; i++) {
            QRgb pixel = image.pixel(i, j); // 分别存放在不同通道的数组中
            r[count] = qRed(pixel);
...
void splitImageChannel(QImage &image, uchar *rgb)
{
    ...
    for (int j=0; j<height; j++) {
        for (int i=0; i<width; i++) {
            QRgb pixel = image.pixel(i, j);                      // {00_r,00_g,00_b,10_r,10_g,10_b,...,wh_r,wh_g,wh_b}
            rgb[j*3*width+3*i] = (uchar)qRed(pixel);             // c*w*h c=3 与上一个函数不同
...
QImage calculateHistogram(QImage &image, ImageChannel channel)
{
    ...
    switch (channel) {
        case ImageChannel::Y:
            hist_ior = qRgba(128, 128, 128, 255); // 三个参数分别是红、绿、蓝、透明度
            bits = grayImage.bits();
            break;
        case ImageChannel::R:
...
QImage equalizeHistogramProc(QImage &image)
{
    ...
    for (uchar **p=c; (*p) != 0; p++)
    {
        ...
        for (int i = 1; i < gray_level; i++)
        {
            gray_distribution[i] = gray_distribution[i-1] + hist[i]*1.0f/pixel_num; // 易得gray_distribution[gray_level] = 1
        }
        // 计算均衡后的灰度分布
        memset(gray_equal, 0, gray_level*sizeof(uchar));
        for (int i = 0; i < gray_level; i++)
